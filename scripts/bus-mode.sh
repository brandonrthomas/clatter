...
# relay never types into it (drain by hand with /clat recv). The relay reads mode at wake time, so a
...
case "$want" in auto|manual) ;; *) echo "usage: /clat mode [auto|manual]" >&2; exit 1 ;; esac
if [ "$want" = "$cur" ]; then echo "already $cur"; exit 0; fi

tmp="$f.tmp.$$"
jq --arg m "$want" '.mode=$m' "$f" > "$tmp" && mv "$tmp" "$f"
echo "mode: $cur -> $want  (session $sid)"
[ "$want" = manual ] && echo "  the relay will no longer wake this session — drain it yourself with /clat recv."
